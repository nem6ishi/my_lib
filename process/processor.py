...
class Processor:
  ...
  def select_model(self):
    if self.setting["train_vars"]["model_type"] == "seq2seq":
      self.model = model.seq2seq.Seq2SeqModel(self.setting, self.src_lang, self.tgt_lang)

    elif self.setting["train_vars"]["model_type"] == "transformer":
      self.model = model.transformer.TransformerModel(self.setting, self.src_lang, self.tgt_lang)

    elif self.setting["train_vars"]["model_type"] == "rnn_transformer":
      self.model = model.rnn_transformer.RNNTransformerModel(self.setting, self.src_lang, self.tgt_lang)

    elif self.setting["train_vars"]["model_type"] == "rel_transformer":
      self.model = model.rel_transformer.RelTransformerModel(self.setting, self.src_lang, self.tgt_lang)
  ...
  def prepare_model(self):
    ...
    logger.info("Model ready: step {}".format(self.step))
  ...
  def prediction(self, corpus, batch_size):
    logger.info("Prediction with batch size of {}".format(batch_size))
    batch = util.corpus.ParallelBatch(corpus, batch_size)
