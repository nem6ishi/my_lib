...
import module.rnn_rel_transformer
...
class TransformerModel(torch.nn.Module):
  def __init__(self, setting, src_lang, tgt_lang):
    ...
    self.decoder = module.rnn_rel_transformer.TransformerDecoder(tgt_lang.vocab_size,
    ...
    self.generator = module.rnn_rel_transformer.TransformerGenerator(tgt_lang.vocab_size,
                                                             setting["decoder_vars"]["model_dim"])
  ...
  def decode(self, encoder_outputs, src_mask, max_target_length, reverse_output=False):
    ...
    for i in range(1, max_target_length):
      decoder_output, hidden = self.decoder(decoder_word_outputs[:, i-1:i], encoder_outputs, src_mask, time_step=i-1, layer_cache=layer_cache, hidden=hidden)
