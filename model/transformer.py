...
class TransformerModel(torch.nn.Module):
  ...
  def translate(self, batch, max_length, beam_size=1, topk_greedy=3, reverse_output=False):
    ...
    if beam_size>1:
      ...
      word_outputs, prob_outputs = self.decode_with_topk_first_words(outputs,
                                                                     batch.src_batch.masks,
                                                                     max_length,
                                                                     topk=topk_greedy)
    else:
  ...
  def decode_with_topk_first_words(self, encoder_outputs, src_mask, max_target_length, topk=3):

    batch_size = encoder_outputs.size(0)
    assert batch_size == 1

    encoder_outputs = encoder_outputs.expand(topk, -1)
    ...
    for i in range(1, max_target_length):
      ...
      if i==0:
        likelihood, index = generator_output.data.topk(topk)
        print(index)
        exit()
        index = index.squeeze(1)
      else:
